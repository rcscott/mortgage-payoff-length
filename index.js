var app = new Vue({
  ...
  data: {
    mortgageAmount: 290000,
    interestRate: 3.50,
    monthlyPaymentMin: 1300,
    monthlyPaymentMax: 2300,
  ...
  methods: {
    calculatePayments: function () {
      ...
      this.paymentsData = _.map(paymentRange, function (paymentAmount) {
        let mortgageBalance = this.mortgageAmount;
        ...
        while (mortgageBalance > 0) {
          paymentCount += 1;
          mortgageBalance = mortgageBalance + (mortgageBalance * monthlyInterestRate);

          if (paymentAmount > mortgageBalance) {
            finalPayment = mortgageBalance;
          }

          mortgageBalance -= paymentAmount;
        }
